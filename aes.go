...
func EncryptAESString(plaintext, password string) (string, error) {
	...
	ciphertextAndNonce, err := EncryptAES([]byte(plaintext), key)
	if err != nil {
		return "", err
	}
...
func DecryptAESString(base64CiphertextAndNonceAndSalt, password string) (string, error) {
	// Decode the base64.
	...
	plaintext, err := DecryptAES(ciphertextAndNonce, key)
	if err != nil {
		return "", err
	}
...
func DecryptAES(ciphertextAndNonce, key []byte) ([]byte, error) {
